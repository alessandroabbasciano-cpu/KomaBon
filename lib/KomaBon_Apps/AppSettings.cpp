...
void AppSettings::handleInput(InputAction action) {
    // SHIELD: During calibration, ignore INPUT_SELECT generated by KEY1/JoyCenter.
    // The only way to abort is using the physical KEY3 (INPUT_BACK).
    if (_screen == SCREEN_JOYCAL) {
        if (action == INPUT_BACK || action == INPUT_GO_TO_MAIN_MENU) {
            _screen = SCREEN_MAIN;
            _needsRedraw = true;
        }
        return; // Ignore all other inputs to prevent accidental exit
    }
...
void AppSettings::update() {
    ...
    if (_screen == SCREEN_JOYCAL && _joyCalStep < 5) {
        ...
        if (_joyCalWaitingRelease) {
            // Wait until user lets go of the joystick (ADC floats back up)
            if (raw > 3800) _joyCalWaitingRelease = false;
        } else {
            if (raw < 3800) {                            // Joystick is pressed
                if (abs(raw - _joyCalLastRaw) < 150) {   // Voltage is stable
                    if (now - _joyCalHoldStart > 1200) { // Held steady for 1.2 seconds

                        // Lock in the value
                        _joyCalValues[_joyCalStep] = raw;
                        _joyCalStep++;
                        _joyCalWaitingRelease = true; // Force user to release
                        _needsRedraw = true;

                        if (_joyCalStep == 5) {
                            Serial.println("AppSettings: Calibration completed. Values:");
                            for (int i = 0; i < 5; i++)
                                Serial.printf("[%d]: %d\n", i, _joyCalValues[i]);
                            // TODO: Save to SettingsStore / KomaBonFS
                        }
                    }
                } else {
                    // Voltage is changing (user is moving the stick or contact is bouncing)
                    _joyCalLastRaw = raw;
                    _joyCalHoldStart = now;
                }
            } else {
                // Nothing pressed, reset the hold timer
                _joyCalHoldStart = now;
                _joyCalLastRaw = 4095;
            }
        }
